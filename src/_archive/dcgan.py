...
class DCGAN(nn.Module):
    ...
    def __init__(
        ...
        Args:
            resize = input resolution window
            batch_size = samples per batch
            data_dir = location to extract data
            url = host of data source, expects .zip file
            n_G = number of output channels from generator network
            n_D = number of input channels for discriminator network
        """
...
def main(url="http://d2l-data.s3-accelerate.amazonaws.com/pokemon.zip"):
    ...
    if True:
        ...
        assert net.net_G(x).shape == torch.Size([1, 3, 64, 64]), "error in Generator resolution."

        # Verify that the 64x64 input results in a single value
        x = torch.zeros((1, 3, 64, 64))
        assert net.net_D(x).shape ==  torch.Size([1, 1, 1, 1]), "error in Discriminator resolution."

    training(net.net_D, net.net_G, device, net.data_iter, num_epochs=10)
